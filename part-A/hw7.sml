...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(deltaX,deltaY,e)=> case eval_prog(e,env) of
                                     NoPoints => NoPoints
                                   | Point(x,y)=> Point(x+deltaX,y+deltaY)
                                   | Line(m,b)=> Line(m,(b+deltaY - m*deltaX))
                                   | VerticalLine(x)=> VerticalLine(x+deltaX)
                                   | Intersect(e1,e2)=> eval_prog(Shift(deltaX,deltaY,intersect(e1,e2)),env)
...
fun preprocess_prog e =
   case e of 
    LineSegment(x1,y1,x2,y2)=> (case real_close_point (x1,y1) (x2,y2) of 
                                true=> Point(x1,y1)
                              | false => if y2<y1 andalso (x2<x1 orelse real_close(x1,x2))
